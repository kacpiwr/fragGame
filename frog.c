/*
	How-to-use ncurses: demo game CATCH THE CAR (windows, moving objects and timing).
...
#define PASS_TIME	20							// 25 s (time to die = no catched car)
#define MVB_FACTOR	2							// move every FRAME_TIME * MVB_FACTOR [ms] CAR
...
#define ROWS		27							// playwin (WIN) parameters
#define COLS		110
#define OFFY		4
#define OFFX		8
...
WINDOW* Start()
{
	...
	init_pair(CAR_COLOR, COLOR_RED, COLOR_WHITE);
...
void draw_map(WIN* W, int bo)
{
	int i, j;
	wattron(W->window,COLOR_GREEN);
	if (bo) box(W->window,0,0);
	for(i = bo; i < W->rows - bo; i+=3)
		for(j = bo; j < W->cols - bo; j++)
			mvwprintw(W->window,i,j,"-");
}
...
void Print(OBJ* ob)
{
	...
	for(int i = 0; i < ob->height; i++)
		mvwprintw(ob->win->window, ob->y+i, ob->x,ob->shape[i]);
										...
										// common function for both: Catcher and Car
...
//	OBJ* ob    = new OBJ;								// C++
	...
	strcpy(ob->shape[0],"0 0");
	strcpy(ob->shape[1],"###");

	InitPos(ob,(ob->win->cols)/ 2,(ob->win->rows)-4);
	ob->xmin   = 1;
	ob->xmax   = w->cols - 1;
	ob->ymin   = 1;
	ob->ymax   = w->rows - 1;
...
void MoveCatcher(OBJ* ob, char ch, unsigned int frame)
{
	...
	{
		switch( ch ) {
			case 'w': Show(ob,0,-3);	break;
			case 's': Show(ob,0,3); 	break;
...
int MainLoop(WIN* status, OBJ* catcher, OBJ* car, TIMER* timer) 			// 1: timer is over, 0: quit the game
{
	int ch;
	int pts = 0;
	while ( (ch = wgetch(status->window)) != QUIT )					// NON-BLOCKING! (nodelay=TRUE)
	{
		...
		{
			if (ch == 'b') { ChangeColors(catcher); CleanWin(catcher->win,1); Show(catcher,0,0); Show(car,0,0); }
			else MoveCatcher(catcher, ch, timer->frame_no);
		}
		MoveCar(car, catcher->x + catcher->width/2, catcher->y + catcher->height/2,timer->frame_no);
		if (Collision(catcher, car))
			{ Restart(catcher,car); pts++; }				// for testing purposes
...
int main()
{
	...
	OBJ* cat = InitFrog(playwin,CATN_COLOR, CATR_COLOR);
	OBJ* car = InitCar(playwin,CAR_COLOR, BALR_COLOR);
	...
	int result;
	if ( (result = MainLoop(statwin, cat, car, timer)) == 0)  EndGame("You have decided to quit the game.",statwin);
	else
	{
		char info[100];
